...
export default function AdminPage() {
  ...
  useEffect(() => {
    ...
    if (savedNumbers) {
      ...
      const defaultNumbers: WhatsAppNumber[] = [
        { id: "1", number: "33123456789", label: "Support Principal", isActive: true },
  ...
  const updateNumber = (id: string, field: keyof WhatsAppNumber, value: string | boolean) => {
    setNumbers(numbers.map((num) => (num.id === id ? { ...num, [field]: value } : num)))
  }
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="flex items-center justify-between mb-8">
          <div className="flex items-center gap-4">
            ...
            <h1 className="text-2xl md:text-3xl font-light">Administration Global Love</h1>
          </div>
          <button
            onClick={saveNumbers}
            ...
            {numbers.map((number, index) => (
              ...
              >
                <div className="flex-1">
                  <label className="block text-sm text-white/70 mb-2">Libellé #{index + 1}</label>
                  <input
                    type="text"
                    value={number.label}
                    onChange={(e) => updateNumber(number.id, "label", e.target.value)}
                ...
                <div className="flex-1">
                  <label className="block text-sm text-white/70 mb-2">Numéro WhatsApp</label>
                  <input
                    type="text"
                    value={number.number}
                    onChange={(e) => updateNumber(number.id, "number", e.target.value)}
                    className="w-full px-3 py-2 bg-white/10 border border-white/20 rounded-lg text-white placeholder-white/50 focus:outline-none focus:border-white/40"
                ...
                <div className="flex items-end gap-2">
                  <label className="flex items-center gap-2 text-sm">
                    <input
                      ...
                      onChange={(e) => updateNumber(number.id, "isActive", e.target.checked)}
                      className="rounded"
                    />
                    Actif
                  </label>
        ...
        <div className="bg-white/5 backdrop-blur-sm rounded-2xl p-6 border border-white/10">
          ...
          <div className="space-y-4">
            <div className="p-4 bg-blue-500/10 border border-blue-500/20 rounded-lg">
              <h3 className="font-medium text-blue-300 mb-2">Numéros Actifs ({activeNumbers.length}/3)</h3>
              {activeNumbers.length > 0 ? (
                <ul className="space-y-2">
                  {activeNumbers.map((num) => (
                    <li key={num.id} className="flex items-center gap-2 text-sm">
                ...
                <p className="text-white/60 text-sm">Aucun numéro actif configuré</p>
              )}
            </div>

            <div className="p-4 bg-yellow-500/10 border border-yellow-500/20 rounded-lg">
              <h3 className="font-medium text-yellow-300 mb-2">Fonctionnement</h3>
              <p className="text-white/70 text-sm">
                Quand un visiteur clique sur "Contactez-nous", le système choisira aléatoirement un des numéros actifs
                pour répartir les contacts. Assurez-vous d'avoir au moins un numéro actif configuré.
              </p>
            </div>

            {activeNumbers.length > 0 && (
              <div className="p-4 bg-green-500/10 border border-green-500/20 rounded-lg">
                <h3 className="font-medium text-green-300 mb-2">Test de Redirection</h3>
                <button
                  onClick={() => {
                    const randomNumber = activeNumbers[Math.floor(Math.random() * activeNumbers.length)]
                    window.open(`https://wa.me/${randomNumber.number}?text=Test%20depuis%20l'admin`, "_blank")
                  }}
